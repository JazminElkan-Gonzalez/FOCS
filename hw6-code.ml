...
| ([],[])-> true
| (c::cl,t::tntl) -> if t = T c then found cl tntl else false
| ([],tntl) -> false
| (cl, []) -> false;;


let applyRule (replace,out) st =
  let rec moveDown left right = match right with
    [] -> []
    | current::right ->  
        if current = N replace 
          then (left@out@right)::(moveDown (left@[current]) right) 
          else (moveDown (left@[current]) right)  
  in
 moveDown [] st;;

let rec applyAllRules rules st = match rules with
[] -> []
| rule::rules -> (applyRule rule st)@applyAllRules rules st;; 
